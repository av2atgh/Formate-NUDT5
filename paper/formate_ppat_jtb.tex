...
\subsection{A coding discrepancy in the published model}

While reconciling the published code against its specification we found that the
cytosolic serine-catabolism term is added to the formate balance rather than to
the CHO-THF balance, routing cytosolic one-carbon flux through the formate pool.
This inflates predicted formate and moves the formate overflow threshold from
$42\%$ to $14\%$ of the FTHFS rate in the extended model (Fig.~\ref{fig:route}),
placing cytosolic flux in the position of a formate source --- in tension with the
conclusion of \citet{oizel2020} that the mitochondrial pathway is the one
required for overflow. Making FTHFS reversible confirms that cytosolic units
reach formate only negligibly (the reaction runs far from equilibrium at
$K_{eq}=20$--$61$; \citealp{himes1962}), so the corrected routing, not the coded
one, is the appropriate baseline. This does not affect the AICAR prediction, for
which the cytosolic pathway is inactive.
