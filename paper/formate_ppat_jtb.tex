...
de novo synthesis of purine and thymidine nucleotides. Serine is the dominant
one-carbon donor: its third carbon is oxidised, chiefly in the mitochondrion, to
formate, which is exported to the cytosol, condensed onto tetrahydrofolate by
10-formyl-tetrahydrofolate synthetase (FTHFS), and used to formylate the growing
purine ring at two positions \citep{ducker2017,tibbetts2010}. When mitochondrial
one-carbon output exceeds the biosynthetic demand, the excess is released as
extracellular formate --- ``formate overflow'' \citep{meiser2018}.
...
We take this opportunity to correct an error in the published implementation,
which we adopt in the corrected form throughout. The specification of
\citet{oizel2020} routes the third carbon of serine to the folate pool by two
distinct paths: the mitochondrial path, in which serine is oxidised to free
formate that is then condensed onto tetrahydrofolate by FTHFS, and the cytosolic
path, in which SHMT1 produces CH$_2$-tetrahydrofolate directly, with no free
formate intermediate. The cytosolic term therefore belongs in the CHO-THF
balance and not in the formate balance. In the published code, however, it is
added to the formate balance, so that cytosolic serine catabolism is made to act
as a source of free formate.

The consequence is not cosmetic. Routing cytosolic flux through the formate pool
inflates the predicted intracellular formate and moves the formate overflow
threshold from $42\%$ to $14\%$ of the FTHFS rate (Fig.~\ref{fig:route}), placing
cytosolic one-carbon metabolism in the position of a formate source. This is in
tension with the central conclusion of \citet{oizel2020} that the mitochondrial
path is the one required for formate overflow. One might ask whether the coded
routing is defensible as an approximation, since the FTHFS reaction is reversible
and cytosolic CHO-THF can in principle return to formate; allowing FTHFS to run
reversibly (Sec.~\ref{sec:extended} and Appendix~\ref{app:model}) shows that it
cannot, as the ATP-driven reaction runs far from equilibrium
($K_{eq}=20$--$61$; \citealp{himes1962}) and the back-flux to formate is
negligible. We therefore adopt the corrected routing --- cytosolic serine
catabolism into the CHO-THF balance --- as the baseline. The correction does not
affect the central prediction of this work (Sec.~\ref{sec:prediction}), which is
evaluated with the cytosolic path inactive.
...
equilibrium and is structurally low ($2$--$13~\mu$M); were free AMP nearer
$100$--$300~\mu$M in these cells, the glue's AMP arm would engage and the
near-constant-$\theta$ regime would not hold. This single quantity, obtainable
from existing adenine-nucleotide data, would most change the analysis. Nine of the
thirteen added parameters are assumed, and the absolute AICAR level is
consequently unconstrained (\S\ref{sec:sens}); only its sign and shape are
predicted. The glue also accepts IMP, GMP and AICAR itself as ligands
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{fig4_routing.pdf}
\caption{Cytosolic one-carbon routing (cyan condition,
$f_{S,\mathrm{CHO}}=0.5$). (a)~Intracellular formate; (b)~formate release, the
overflow threshold being the zero crossing. The reversible-FTHFS model (dotted)
tracks the specified routing (dashed), not the coded one (solid).}
\label{fig:route}
\end{figure}
